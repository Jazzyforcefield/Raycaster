...
int print_values() {
  ...
  for (int i = 0; i < mtlc; i++) {
    std::cout << "mtlcolor: " << mtlcolor[i].r << " " << mtlcolor[i].g << " ";
    std::cout << mtlcolor[i].b;
    std::cout << std::endl;
  }
...
int main(int argc, char ** argv) {
  ...
  VectorType ul = VectorType(center.x + n.scalar(focal).x - un.x + vn.x, center.y +
                             n.scalar(focal).y - un.y + vn.y, center.z + n.scalar(focal).z -
                             un.z + vn.z);

  VectorType ur = VectorType(center.x + n.scalar(focal).x + un.x + vn.x, center.y +
                             n.scalar(focal).y + un.y + vn.y, center.z + n.scalar(focal).z +
                             un.z + vn.z);

  VectorType ll = VectorType(center.x + n.scalar(focal).x - un.x - vn.x, center.y +
                             n.scalar(focal).y - un.y - vn.y, center.z + n.scalar(focal).z -
                             un.z - vn.z);

  VectorType lr = VectorType(center.x + n.scalar(focal).x + un.x - vn.x, center.y +
                             n.scalar(focal).y + un.y - vn.y, center.z + n.scalar(focal).z +
                             un.z - vn.z);
